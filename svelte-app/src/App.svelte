<script>
  ...
  import { gameData, clerkOpen, activeTab, agentFeeds, agentStatuses, loadGame } from './stores/game.js';
  import { agentColor, shortTime } from './lib/utils.js';
  import PublicChat from './components/PublicChat.svelte';
  import AgentPanel from './components/AgentPanel.svelte';
  import AgentEvent from './components/AgentEvent.svelte';
  import ChatMessage from './components/ChatMessage.svelte';
  import PlaybackBar from './components/PlaybackBar.svelte';
  import { currentIdx, visibleEvents } from './stores/game.js';
...
<div class="topbar" class:clerk-open={$clerkOpen}>
  <div class="topbar-left">
    ...
    <div class="tab-bar">
      ...
      <button class="tab-btn" class:active={$activeTab === 'scores'} on:click={() => switchTab('scores')}>Scores</button>
...
<div class="main-area" class:clerk-open={$clerkOpen}>
  ...
  {:else if $activeTab === 'scores'}
    <div class="scores-layout">
      <div class="loading">Scores tab — coming soon</div>
    </div>
  {:else if $activeTab === 'rules'}
    <div class="rules-layout">
      <div class="loading">Rules tab — coming soon</div>
    </div>
  {/if}
</div>
...
<style>
  ...
  .scores-layout, .rules-layout { flex: 1; padding: 24px 32px; overflow-y: auto; }
  .loading {
